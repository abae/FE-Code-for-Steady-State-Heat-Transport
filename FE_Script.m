...
for manufacturedmethod = 1:5
    disp(manufacturedmethod);
    elemsizeX = elemsizeX*2;
    elemsizeY = elemsizeY*2;
    ...
    Treal = zeros(length(NodeCoord),1);
    TsolT = 0;
    Tsol = 0;
    for i = 1:length(NodeCoord)
        Treal(i) = NodeCoord(i,1)^2 + NodeCoord(i,2)^2 + 2*NodeCoord(i,1)*NodeCoord(i,2);
        TsolT = TsolT + (Treal(i)-T(i))^2;
        Tsol = Tsol + (Treal(i))^2;
    end
    TsolT = sqrt(TsolT);
    Tsol = sqrt(Tsol);
